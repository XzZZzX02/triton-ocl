...
int main() {
  ...
  try {
    ...
    // Launch kernel with local_accessor
    sycl::nd_range<3> launch_config(
        sycl::range<3>(num_blocks * BLOCK_SIZE, 1, 1), // global
        sycl::range<3>(BLOCK_SIZE, 1, 1)               // local
    );

    q.submit([&](sycl::handler &h) {
       // Allocate local memory (shared within work-group)
       sycl::local_accessor<float, 1> local_x(sycl::range<1>(BLOCK_SIZE), h);
       sycl::local_accessor<float, 1> local_y(sycl::range<1>(BLOCK_SIZE), h);

       h.parallel_for(launch_config, [=](sycl::nd_item<3> item) {
         // All work-items execute the kernel
         add_kernel(item, d_x, d_y, d_output, N, local_x, local_y);
       });
     }).wait();
